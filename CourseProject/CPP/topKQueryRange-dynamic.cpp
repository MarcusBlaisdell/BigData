...
list <theViewsStruct> insertSortViewsList (theViewsStruct myViews, list <theViewsStruct> viewsList, int K)
{
  ...
  if (viewsList.size () == 0)
  {
    ...
    // insert at end only if list size is less than K
    // otherwise, if list is greater than K and our value should
    // be placed at the end, then it is less than the Kth smallest
    // value and therefore irrelevant so we don't need to record it
  ...
  } // end only insert at end if list size less than K
  ...
  list <theViewsStruct>::iterator it = viewsList.begin ();
  {
    ...
    } // end only look at first K values
...
int main(void)
{
  ...
  while (line[0] != 0)
  {
    ...
    while (res->next())
    {
      ...
      //cout << "myViews.theVideoID: " << myViews.theVideoID << " - myViews.theViews: " << myViews.theViews << endl;
  ...
  /*
  //for (list <int>::iterator itr = k_views.begin (); itr != k_views.end (); itr++)
  list <int>::iterator itr = k_views.begin ();
  for (i = 0; i < K; i++)
  {
    theKth = *itr;
    itr++;
  } // end iterate to the kth element
  */
  ...
  list <theViewsStruct>::iterator it = viewsList.begin ();
  {
    //cout << it->theVideoID << " : " << it->theViews << "thekth: " << theKth << endl;
    //if (it->theViews >= theKth)
    for (i = 0; i < K; i++)
    {
      ...
      {
        break;
      } // end if fewer than K, break;

    } // end only print if above the smallest value in the top k list
  ...
  cout << "totalTime: " << float (totalEnd - totalStart) / 1000000 << endl;
