...
void printRates (list <theRateStruct> ratesList, ofstream &outFile, int K)
{
  ...
  cout << "videoID:  Rate" << endl;
    ...
    // if there are fewer than K elements in our list,
    // only print what we have:
...
int main(void)
{
  ...
  while (line[0] != 0)
  {
    ...
    res = stmt->executeQuery("select distinct videoID, rate, ratings, views from " + line + " where category = \"Music\";");
    ...
    while (res->next())
    {
      ...
      theRateFloat = strtofloat (theRate);
      theRatings = res->getString (3);
      theRatingsFloat = strtofloat (theRatings);
      theViews = res->getString (4);
      theViewsFloat = strtofloat (theViews);
      myRates.theRate = ( (theRateFloat * theRatingsFloat) / theViewsFloat );
