...
// insert views into table:

void insertViews (string line)
{
  sql::Driver *driver;
  sql::Connection *con;
  sql::Statement *stmt;
  ...
  stmt->execute("insert into viewsTable select distinct videoID, views from " + line);
...
} // end insertViews function

  // insert Rated into table:

void insertRated (string line)
{
  sql::Driver *driver;
  sql::Connection *con;
  sql::Statement *stmt;
...
} // end insertRated function

// get Rated Data function
...
} // end getRated function

// delete Rated data from table:
...
void getViews ()
{
  ...
  res = stmt->executeQuery("select distinct videoID, views from viewsTable order by views desc limit 20;");
  ...
  while (res->next())
  {
    ...
    //cout << "theViews: " << theViews << " - myViews.theViews: " << myViews.theViews << endl;
    cout << "theVideoID: " << theVideoID << " - theViews: " << theViews << endl;

  } // end while loop

  stmt = con->createStatement();

  stmt->execute("delete from viewsTable where views >= 0");
  //stmt->execute("insert into " + theTable + " (videoID, uploader, age, \
...
} // end getViews function

// delete Views data from table:
...
} // end deleteViews function

// insert Category

void insertCategory (string line, list<TheSUMs> runningSums, TheSUMs mySum)
{
  sql::Driver *driver;
  sql::Connection *con;
  sql::Statement *stmt;
...
int main(void)
{
  ...
  */
  ...
  outFile.open ("topK-All_0.txt", ios::out);
  timeFile.open ("topK-All_0_times.txt");
  ...
  while (line[0] != 0)
  {
    ...
    //{
      ...
      insertCategory (line, runningSums, mySum);
      insertViews (line);
      insertRated (line);
